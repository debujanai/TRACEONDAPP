...
class TokenSecurity:
    ...
    def get_top_traders(self, contract_address: str) -> dict:
        """
        Get information about the top traders for a specific token.
        ...
        url = f"{self.BASE_URL}/defi/quotation/v1/tokens/top_traders/eth/{contract_address}"
        return self.make_request_with_retry(url)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(json.dumps({"error": "Missing arguments. Usage: python token_security.py <method> <contract_address>"}))
        sys.exit(1)
