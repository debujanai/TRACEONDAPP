...
contract {{TOKEN_NAME}} is ERC20{{INHERITANCE}} {
    ...
    constructor() ERC20("{{TOKEN_NAME}}", "{{TOKEN_SYMBOL}}") {{CONSTRUCTOR_MODIFIERS}} {
        {{CONSTRUCTOR_BODY}}
...
function generateContractCode(
  ...
  if (features.includes('Mintable') || features.includes('Pausable') || features.includes('Flash Minting')) {
    if (!features.includes('Access Control')) {
      ...
      // Don't automatically transfer ownership - it's already handled by the Ownable contract
      // The Ownable constructor in OpenZeppelin automatically sets msg.sender as the owner
    }
  }
