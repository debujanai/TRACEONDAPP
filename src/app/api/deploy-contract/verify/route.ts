...
export async function POST(request: Request) {
  try {
    const { address, constructorArguments, contractCode, chainId, network } = await request.json();
    ...
    console.log('Verifying contract:', {
      ...
      contractName: contractCode.match(/contract\s+(\w+)/)?.[1] || '',
    });

    // Validate required fields
    if (!address || !contractCode || !chainId) {
      console.error('Missing required fields:', { address, contractCode: !!contractCode, chainId });
    ...
    const contractName = contractCode.match(/contract\s+(\w+)/)?.[1] || '';
    ...
    try {
      ...
      const flattenedCode = await flattenContract(contractCode);
      ...
      sourceCode = contractCode; // Fall back to original code
    }
    ...
    if (result.status === '1') {
      ...
      if (retryCount === maxRetries - 1 && result?.status !== '1') {
        ...
        verificationData.sourceCode = sourceCode;
